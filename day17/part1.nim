...
proc enumIntersections(board:Board):seq[Point] =
  for key in board.tiles.keys():
    var intersection = true
    for d in [North, East, West, South]:
      if board.tileAt(key.plus d) != '#':
        intersection = false
        break
    if intersection:
      result.add(key)

proc answer(board:Board):int =
  for i in board.enumIntersections:
    result += i.x * i.y

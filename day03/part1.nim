...
proc makeHashSet(instructions:string):HashSet[Point] =
  init(result)
  var cursor:Point
...
proc find_cross(w1, w2: string):int =
  var line1 = makeHashSet(w1)
  var line2 = makeHashSet(w2)
  let common = toSeq(line1 * line2)
  result = min(common.mapIt(it.distance))

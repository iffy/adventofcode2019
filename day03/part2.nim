...
proc makeTable(instructions:string):Table[Point, int] =
  ...
  for x in instructions.split(","):
    ...
    of 'R':
      ...
      raise newException(CatchableError, "Invalid direction")
    for i in 1 .. mag:
      steps.inc()
...
proc find_cross(w1, w2: string):int =
  var line1 = makeTable(w1)
  var line2 = makeTable(w2)
  var h1 = toHashSet(toSeq(line1.keys()))
  var h2 = toHashSet(toSeq(line2.keys()))
  let common = toSeq(h1 * h2)
  result = min(common.mapIt(line1[it] + line2[it]))
